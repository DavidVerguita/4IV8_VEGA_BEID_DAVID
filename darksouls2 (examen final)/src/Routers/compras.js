...
// ============================================================
// REGISTRAR UNA NUEVA COMPRA (CORREGIDO)
// ============================================================
router.post('/', async (req, res) => {
    try {
        ...
        if (!usuario_id || !producto_id || !cantidad || parseInt(cantidad) < 1) {
            return res.status(400).json({ status: 'error', message: 'Datos de la compra incompletos o inválidos' });
        }

        // 2. BUSCAR EL PRECIO DEL PRODUCTO EN LA BD
        const [productos] = await db.execute('SELECT precio FROM productos WHERE id = ?', [producto_id]);
        
        if (productos.length === 0) {
            return res.status(404).json({ status: 'error', message: 'El producto seleccionado no existe' });
        }

        // Extraemos el precio real del producto
        const precio_unitario = productos[0].precio;
        
        // Calculamos el total de la compra
        const total = precio_unitario * parseInt(cantidad);

        // 3. INSERTAR EN LA TABLA COMPRAS CON TODOS LOS CAMPOS REQUERIDOS
        const [resultado] = await db.execute(
            'INSERT INTO compras (usuario_id, producto_id, cantidad, precio_unitario, total) VALUES (?, ?, ?, ?, ?)',
