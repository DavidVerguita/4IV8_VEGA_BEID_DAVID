...
router.post('/', async (req, res) => {
    try {
        ...
        // --- ¡AQUÍ ENTRA LA VALIDACIÓN DE CORREO DUPLICADO! ---
        const [existe] = await db.execute('SELECT id FROM usuarios WHERE email = ?', [email]);
        
        if (existe.length > 0) {
            // Si el arreglo tiene algo, significa que el correo ya existe
        ...
        // Si no existe, tu código sigue ejecutándose de forma normal:
        const [resultado] = await db.execute('INSERT INTO usuarios (nombre, email) VALUES (?, ?)', [nombre, email]);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        // Buscamos si existe alguien con ese email, PERO que tenga un ID diferente al que estamos editando
        const [existe] = await db.execute('SELECT id FROM usuarios WHERE email = ? AND id <> ?', [email, id]);
        
        if (existe.length > 0) {
            return res.status(400).json({ 
                status: 'error', 
                message: 'Este correo electrónico ya pertenece a otro usuario' 
            });
        }
        // ------------------------------

        // Si está libre, haces el UPDATE normal:
